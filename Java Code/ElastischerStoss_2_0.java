...
public class ElastischerStoss_2_0 extends PApplet {

    public static void main(String[] args) {

        PApplet.runSketch(new String[]{"Grab Ball"}, new ElastischerStoss_2_0());
    }

    //setup
    final int WINDOW_SIZE_X = 500;      //horizontal window size
    final int WINDOW_SIZE_Y = 500;      //vertical window size
    int frameDelay = 1;
    ...
    int ballMaxSize = 50;               //maximum random ball size
    ...
    double airFriction = 2;             //constant speed reduction by percentage
    ...
    Ball baelle[] = new Ball[20];       //number of balls
...
//    };
    ...
    public void keyAction() {
        if (keyUp == true) baelle[0].ySpeed -= movingSpeed;
        if (keyDown == true) baelle[0].ySpeed += movingSpeed;
        if (keyLeft == true) baelle[0].xSpeed -= movingSpeed;
        if (keyRight == true) baelle[0].xSpeed += movingSpeed;
        if (keySpace == true) baelle[0].ySpeed -= 10*movingSpeed;
        if (keyT == true) scaleSpeed(baelle[0], 1.2);
    }
